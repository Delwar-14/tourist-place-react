  ...
  const handleSubmit = (event) => {
    ...
    userSchema
      ...
      .then(async () => {
        ...
        form.append('images', image.value);

        if (!id) await createPlace(form);
        else
          updatePlace(
            {
              name: name.value,
    ...
    <Form style={AddNewPlaceStyle} onSubmit={handleSubmit}>
      ...
      <Form.Group className="input-form" as={Row}>
        <Col className="btn-div">
          <button className="btn btn-success submit-reset-btn" type="submit">
            submit
          </button>
          <button
            onClick={resetBtnHandler}
            className="btn btn-warning submit-reset-btn center"
            type="reset"
          >
            reset
          </button>
